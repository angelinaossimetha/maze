...
#|
   Angelina Ossimetha
   Rachael Schmidt
   Project 2: Magical Maze Reflection

   This project was designed to give you practice with organizing and 
   manipulating structured data. Answer the following questions about these 
   topics in a block comment at the bottom of magicmaze.arr.

1) Our support code represented the maze layout (walls vs floors) as
a list-of-lists of strings, rather than a table. What were the advantages
and disadvantages of this choice?

   > PROS

   – Easy to calculate the positions of the walls and ground, or extract
     anything (L.get).

   – We don't have the luxury of using transformative functions like L.map
     on a list-of-list of strings.

   – Easy to stack layers of wall and ground on top of each other
     (ex: beside, above, and link)).

   > CONS

   – Using recursion to stack list pieces on top of each other might take
     longer than using a table function (this relates to ideas of runtime;
     this might not be a linear task).

   – Using list functions like index can be easy to pick out specific maze
     rows and columns we want.

   – Getting individual elements from the inner lists is lengthy and takes
     longer than if it was just a list of strings.

   – Using recursion and image functions like below/above makes producing 
     the image easy and less manual work.

2) In this project, you worked with the maze in three formats: the 
Google Sheet with the configuration of walls, the list-of-lists version, 
and the image itself. For each representation, briefly describe what it
is good and bad for.

Google Sheet Format:

   Pros: It's a nice visual representation of the maze that allows us to 
   easily see where walls/walkable areas are in the forms of "x"s and
   "o"s, and the position of the walls and walkable areas (in the form of
   our Posns).

   Cons: 	Changing the data to fit our needs requires loading the 
   Google Sheet in Pyret and creating Pyret functions (helper functions)
   to manipulate it; it's not very convenient in the long run.

Lists-of-Lists Format:

   Pros: It's easy to stack pieces of the maze on top of each other, because 
   they occur sequentially in the list-of-list (recursion). We can put 
   wall-detection (not walking on gray tiles; walking only on green) in 
   these list functions. With the list-of-lists format, we created the 
   is-a-wall function that takes in this list-of-lists and a Posn, and
   it is easy to extract data and see if a Posn is a wall or not.

   Cons: Obviously, this format doesn't produce an actual image of a maze.
   Our final goal is a visual image of a maze; to do this, we had to create
   multiple helper functions involving recursion to do so.

Image Format:

   Pros: Now, we can visualize the maze.

   Cons: If we just had an image without any underlying organization 
   (i.e., if it wasn't encoded by a list of a list), it would be 
   really hard to put things like wall-respecting in (it is difficult
   to do anything with the maze if we don't know how the coordinates map
   out)! Additionally, we don't know where to put fruits/portals, etc.

3) Describe one key insight that each partner gained about programming
or data organization from working on this project.

   > Rachael: The way we recur things such as lists or our own data-types
   can greatly affect the runtime of our programs. Additionally, figuring
   out wall-detection initially required three-to-four different functions
   that converted our list-of-list of strings to a list of Posns 
   representing walls. We implemented wall-respecting by seeing if a
   character walked to a Posn that was within this list of Wall Posns. 
   We instead created is-a-wall that that shows if a tile is an "x." 
   We then implemented this in key-pressed. This was way simpler and saved
   us even more time!

   > Angelina: Using recursion and other helper functions makes programming
   efficient and nicer for other people to understand. Using docs and where
   block are very important when dealing with lots of data and a big scale
   project. Important to list task before executing project. 

4) Describe one or two misconceptions or mistakes that you had to
work through while doing the project.

   > We were under the impression that a position of (2, 2) scaled to the 
   maze's size would set us in the middle of our maze's green path, when 
   in fact, it put us on the edges of maze blocks: We recognized later
   that the Posn datatype will go to the corners of our maze blocks,
   and not square in the center of a maze block. 

   > Dealing with key-pressed functions was difficult. The shifted
   x-and-y coordinates made math difficult (ex: instead of going up 
   being y + 1 on a normal graph, in our key-pressed function, it 
   was y - 1!). Accounting for this difference in our math took a lot of
   time.

5) State one or two follow up questions that you have about programming
or data organization after working on this project. 

   > How to make code more efficient and faster (ex: referring back to
   Question 3, Part A)? What are some tips for thinking about efficiency
   and runtime? 

   > After looking at our code, what are ways that we could have made our
   code more efficient?

   > When is the issue of runtime applicable? 
